...
>
  ...
  <!-- global variables and functions -->
	...
	<xsl:variable name="nodeset-class"
		select=".//*[     
		   (rdf:type/@rdf:resource='&owl;Class' or rdfs:subClassOf/@rdf:resource or (@rdf:about and rdf:type/@rdf:resource='&owl;subClassOf') or (local-name()='Class' and namespace-uri()='&owl;' and @rdf:about) or (owl:Class and @rdf:about))
  ]" />
	...
	<!-- Put IE into quirks mode -->
		<html>
			...
			<body>
				<xsl:apply-templates />
				<div id="footer">
                                    <p><a href="https://togoid.dbcls.jp/">TogoID</a> is a service developed by the Database Center for Life Science (<a href="https://https://dbcls.rois.ac.jp/">DBCLS</a>) for converting identifiers of databases in life sciences. TogoID ontology (TIO) is developed for semantically annotate relations in the conversion.</p>
				</div>
			</body>
		</html>
	</xsl:template>
	...
	<xsl:template name="url">
		...
		<xsl:choose>
			...
			<xsl:when test="$ns='&tio;'">
				<a href="{concat($ns,$name)}">
					tio:<xsl:value-of select="$name" />
				</a>
			</xsl:when>
	...
	<xsl:template name="prettyUrl">
		...
		<xsl:choose>
			...
			<xsl:when test='starts-with($name, "&tio;")'>
				tio:<xsl:value-of select='substring-after($name, "&tio;")' />
			</xsl:when>
