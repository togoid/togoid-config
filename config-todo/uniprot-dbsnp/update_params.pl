...
WHERE {
  ?org ^up:organism [ 
    a up:Protein ;
    up:annotation/rdfs:seeAlso/up:database db:dbSNP 
  ] .
}";
...
WHERE {
  ?source a up:Protein ;
           ...
           up:annotation/rdfs:seeAlso ?target .
  ?target up:database db:dbSNP .
}";

# regex : req. double escape backslash (e.g. '\d' -> '\\d')
our $SOURCE_REGEX = "http://purl.uniprot.org/uniprot/(.+)";
our $TARGET_REGEX = "http://purl.uniprot.org/dbsnp/(.+)";
