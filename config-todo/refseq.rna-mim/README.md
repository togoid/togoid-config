...
```
% gzip -dc human.*.rna.gbff.gz | ./parse_refseq.rna_gbff.pl --mim
...
* ここでは `mim`, `prefix: http://identifiers.org/mim/` としましたが、 `omim`, `prefix: http://identifiers.org/omim/` と記載している config もあります。
  * ディレクトリ名も `refseq.rna-mim` としましたが、 `refseq.rna-omim` とすべきでしょうか。
* 対応する mim が存在しない場合はそのエントリ自体を出力していないが、それでよいか。
