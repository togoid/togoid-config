...
SELECT  (replace(str(?instruct), "http://purl.jp/10/instruct/", "") as ?instruct_id)   (replace(str(?uniprot), "http://identifiers.org/uniprot/", "") as ?uniprot_id)
FROM <http://med2rdf.org/graph/instruct> 
where {
   ?instruct a bp3:MolecularInteraction ;
      bp3:participant / obo:BFO_0000051 ?uniprot .
   FILTER(CONTAINS(STR(?uniprot, "uni"))
  }
