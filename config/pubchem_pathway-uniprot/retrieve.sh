...
# PubChem pathway IDとUniProt IDのペアを取得する。

ENDPOINT=https://rdfportal.org/pubchem/sparql
WORKDIR=pathway2uniprot # 一時的にリストファイルを保存するディレクトリ
...
PATHWAY_UNIPROT_QUERY_FILE=query_01.rq
# 上記クエリで得られる結果の全件数を取得するクエリのテンプレート。
COUNT_PATHWAY_UNIPROT_QUERY_FILE=query_01_count.rq

if [ ! -e $PATHWAY_UNIPROT_QUERY_FILE ]; then echo "必要なファイルが不足しています。:$PATHWAY_UNIPROT_QUERY_FILE"; exit; fi
if [ ! -e $COUNT_PATHWAY_UNIPROT_QUERY_FILE ]; then echo "必要なファイルが不足しています。:$COUNT_PATHWAY_UNIPROT_QUERY_FILE"; exit; fi
...
CID_TOTAL=$($CURL -sSH "Accept: text/csv" --data-urlencode query@$COUNT_PATHWAY_UNIPROT_QUERY_FILE $ENDPOINT | tail -1)
#echo 取得対象CID数: $CID_TOTAL
if [ $CID_TOTAL -lt $LIMIT ]; then COUNT=0; else 
  COUNT=$(expr $CID_TOTAL / $LIMIT)
fi

for i in $(seq 0 ${COUNT}); do
  QUERY=$(sed -e "$ a OFFSET ${i}000000 LIMIT ${LIMIT}" $PATHWAY_UNIPROT_QUERY_FILE)
