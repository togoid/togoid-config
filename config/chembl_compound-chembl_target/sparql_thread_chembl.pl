...
sub get {
    ...
    ## Endpoint result-limit check, and separate into 10 parts
    if (($#{$json->{results}->{bindings}} + 1) > 0 && ($#{$json->{results}->{bindings}} + 1) % 10000 == 0) {
	my @page = ();
	
        for (my $i = 0; $i < 10; $i++) {
	    my $tmp_query = $query;
	    $tmp_query =~ s/}\s*$/FILTER (REGEX (STR (?source), '${i}\$'))\n}/;
	    $json = &get_req("?query=".uri_escape($tmp_query), $tmp_id);
	    print STDERR "over 1000000 bindings\n" if ($#{$json->{results}->{bindings}} + 1 == 1000000);
